...
class FileManager:
    ...
    def _save_pem_and_optional_der(
        self,
        pem_bytes: bytes,
        pem_name: str,
        ...
        if der_name and der_bytes is not None:
            ...
            if not self._check_overwrite(path_der, interactive):
                raise FileExistsError(
                    f"File {path_der} exists and overwrite was denied"
                )
    ...
    def save_private_key(
        ...
        if not self._check_overwrite(path, interactive):
            raise FileExistsError(
                f"Private key file {path} exists and overwrite was denied"
            )
    ...
    def save_certificate(
        ...
        return self._save_pem_and_optional_der(
            pem_bytes, pem_name, der_bytes, der_name, interactive
        )
...
class CertificateManager:
    ...
    def create_self_signed(
        self, key, subject: x509.Name, san_list: List[x509.GeneralName], days: int = 365
    ) -> x509.Certificate:
        ...
        try:
            ...
            logger.info(
                "Added Subject Key Identifier and Authority Key Identifier extensions"
            )
        except Exception as e:
    ...
    def create_csr(
        self, key, subject: x509.Name, san_list: List[x509.GeneralName]
...
class OPCUAWizard:
    ...
    def prompt_password(question: str) -> str:
        ...
        try:
            ...
            logger.warning(
                "Password input will be visible (prompt_toolkit version doesn't support hidden input)"
            )
    ...
    def run(self):
        ...
        base_path = Path(
            self.prompt("Base path for Siemens trustlist", default="./opcua_certs")
        )
        ...
        if key_size == 4096:
            print(
                "[!] Warning: 4096-bit keys may not be supported by all S7-1500 PLCs. Verify compatibility."
            )
        self.cert_mgr = CertificateManager(key_size)

        mode = self.prompt(
            "Certificate type? (selfsigned/csr)",
            default="selfsigned",
        ...
        if extra_ip:
            try:
                ...
                logger.warning(
                    f"First IP '{extra_ip[0]}' is invalid, will not be used as primary"
                )
        ...
        app_uri = self.prompt(
            "ApplicationUri (urn:hostname:app)", default=f"urn:{hostname}:MyOPCUAClient"
        )

        # Validate URI
        if not self.validate_uri(app_uri):
            print(
                "[!] Warning: ApplicationUri does not look like a valid URI; recommended to start with 'urn:'"
            )
        ...
        # Extra DNS
        extra_dns_raw = self.prompt(
            "Extra DNS entries (comma separated, optional)", default="localhost"
        )
        extra_dns = [d.strip() for d in extra_dns_raw.split(",") if d.strip()]

        export_der = self.prompt(
            "Export DER also? (y/n) [Recommended for S7-1500]",
            default="y",
            completer=self.yesno_completer,
        ).lower() in ("y", "yes")
        if export_der:
            print(
                "[i] DER format is preferred for PLC import and filename conventions align with Siemens expectations"
            )

        password = None
        if self.prompt(
            "Encrypt private key? (y/n)", default="n", completer=self.yesno_completer
        ).lower() in ("y", "yes"):
        ...
        if mode == "selfsigned":
            ...
            self.file_mgr.save_certificate(
                cert, cert_pem, cert_der if export_der else None
            )
        ...
        print("\n=== Next Steps ===")
        print(
            "• Upload client certificate to PLC trusted store via TIA Portal or Web UI"
        )
        print(
            "• Ensure PLC accepts the certificate format (DER recommended for S7-1500)"
        )
        print(
            "• Configure OPC UA client to use the generated private key and certificate"
        )
        if primary_ip:
    ...
    def run_non_interactive(self, args):
        ...
        subject = self.cert_mgr.build_subject(
            args.common_name, args.organization, args.country
        )
        ...
        safe_name = "".join(
            c for c in args.common_name if c.isalnum() or c in ("-", "_")
        )
        key_fname = f"{safe_name}_key.pem"
        cert_pem = f"{safe_name}.pem"
        cert_der = f"{safe_name}.der"
        ...
        self.file_mgr.save_private_key(
            key, key_fname, password=args.password, interactive=False
        )

        if args.mode == "selfsigned":
            cert = self.cert_mgr.create_self_signed(
                key, subject, san_list, args.validity_days
            )
            self.file_mgr.save_certificate(
                cert, cert_pem, cert_der if args.export_der else None, interactive=False
            )
            trusted_copy = self.file_mgr.base / "trusted" / "certs" / cert_pem
            trusted_copy.write_bytes(cert.public_bytes(serialization.Encoding.PEM))
            ...
            csr = self.cert_mgr.create_csr(key, subject, san_list)
            self.file_mgr.save_csr(
                csr, csr_pem, csr_der if args.export_der else None, interactive=False
            )
...
Examples:
  ...
  Generate 4096-bit certificate:
    ...
    parser.add_argument(
        "--hostname", help="Hostname for certificate (default: auto-detect)"
    )
    parser.add_argument("--primary-ip", help="Primary IP address to include in SAN")
    parser.add_argument("--extra-ip", help="Additional IP addresses (comma-separated)")
    parser.add_argument("--extra-dns", help="Additional DNS names (comma-separated)")
    parser.add_argument(
        "--app-uri", help="Application URI (default: urn:hostname:MyOPCUAClient)"
    )
    parser.add_argument(
        "--validity-days",
        type=int,
    ...
    parser.add_argument(
        "--password", help="Private key password (leave empty for no encryption)"
    )
    parser.add_argument(
        "--export-der",
        action="store_true",
